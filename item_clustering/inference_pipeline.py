...
def predict_single_item(new_item_umap, cluster_profiles, confidence_threshold=-999):
    ...
    # ✅ Fix: assign based on confidence_threshold, not whether confidence > 0
    # confidence_threshold=0.0 means "assign to nearest cluster always"
    # confidence_threshold=0.3 means "only assign if confidence >= 0.3"
    if best_confidence >= confidence_threshold:
        assigned_cluster = best_cluster
        final_confidence = best_confidence  # can be negative, that's fine
    else:
        assigned_cluster = -1
