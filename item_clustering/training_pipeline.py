...
class FeatureTransformer:
    ...
    def fit_transform(self, df):
        ...
        # Save norms during fit
        self.block_norms_ = []
        block_start = 0
        for block in transformed_blocks:
            block_end = block_start + block.shape[1]
            norm = np.linalg.norm(X_transformed[:, block_start:block_end], axis=1).mean()
            self.block_norms_.append(norm)
            if norm > 0:
                X_transformed[:, block_start:block_end] /= norm
            block_start = block_end
    ...
    # Apply the saved training norms — NOT recomputed from inference data
        block_start = 0
        for norm, block in zip(self.block_norms_, transformed_blocks):
            block_end = block_start + block.shape[1]
            if norm > 0:
                X_transformed[:, block_start:block_end] /= norm
            block_start = block_end
...
def train_clustering_pipeline(df, preference='conservative'):
    ...
    n_components = min(15, X_transformed.shape[0] - 2)  # never exceed n_samples - 2

    umap_model = umap.UMAP(
        n_components=n_components,
        n_neighbors=min(15, X_transformed.shape[0] - 1),
        min_dist=0.1,
        metric='euclidean',
        random_state=42,
        init="random"
    ...
    # Compute same confidence formula as inference
    confidences = np.zeros(len(labels))
    for i, (label, point) in enumerate(zip(labels, X_umap)):
        if label == -1:
            # outlier — find nearest cluster medoid
            best_conf = -999
            for cluster_id, profile in cluster_profiles.items():
                dist = np.linalg.norm(point - profile['medoid'])
                conf = 1.0 - (dist / profile['threshold'])
                if conf > best_conf:
                    best_conf = conf
            confidences[i] = best_conf
        else:
            # assigned — use its own cluster
            profile = cluster_profiles[label]
            dist = np.linalg.norm(point - profile['medoid'])
            confidences[i] = 1.0 - (dist / profile['threshold'])

    df_clustered['confidence'] = confidences
