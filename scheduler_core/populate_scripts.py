...
from scheduler_core.models import Item, Source, Script

def populate_scripts():
    session = SessionLocal()
    base_dir = os.path.dirname(os.path.abspath(__file__))
    extractor_dir = os.path.join(base_dir, "extractor_modules")
    ...
    for filename in files:
        ...
        script_path = os.path.join("extractor_modules", filename)
        
        # Check both absolute and relative paths for existing scripts
        script = session.query(Script).filter(
            (Script.path == script_path) | 
            (Script.path == os.path.join(extractor_dir, filename))
        ).first()
        ...
        if not script:
            ...
            session.add(script)
        elif os.path.isabs(script.path):
            print(f"Updating absolute path to relative for: {filename}")
            script.path = script_path
